...
$("#qqhao").blur(function() {
	...
	$.ajax({
		url: pjax_url + "api/nic.php?qq=" + $("#qqhao").val(),
		type: "GET",
		dataType: "jsonp",
		success: function(a) {
			if (a.name) {
	...
	if (!lcs.get('tb_wintips_ts') > 0) {
		...
		setTimeout(function() {
			$('body').append('<div class="wintips">\
				<div class="wintips-thumb"><img src="'+ pjaxtheme +'img/flyman.png"></div>     <h2>投稿送5~20元现金奖励</h2>\
				<p>好东西要懂得分享,一人独乐乐是没有意思的</p>\
				<p><a href="tg.html"'+blog_url+'" class="btn btn-info btn-wintips">查看详情</a></p>\ <span etap="wintips_close" class="wintips-close"><i class="fa fa-times-circle-o"></i></span>\
			</div>')
			$('.wintips').fadeIn()
		}, 1000);
	}
...
function copying(){swal("复制成功！", "若要转载请务必保留原文链接！谢谢合作！", "success");}
document.body.oncopy=function(){copying();}